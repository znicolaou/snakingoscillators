...
%Under this  the backward time system is equivalent to the system with negative coupling constants $\sigma\to-\sigma$ and $\beta\to-\beta$. Numerically finding the attractive chimeras in the ring with negative coupling constants reveals a second unstable branch of chimeras in the system with positive coupling constants, which do indeed propagate in the opposite direction as the stable chimeras. Note however that when $\sigma=\beta$, the transformation $t\to -t$, $\theta_n \to \phi_n$, $\phi_n \to \theta_n$ is a symmetry, so we expect the stable and unstable chimera solution branches may coincide at $\sigma=\beta$.
